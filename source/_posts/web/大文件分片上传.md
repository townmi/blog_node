...
我们可以通过[`spark-md5`](https://github.com/satazor/js-spark-md5)生成文件的 MD5。需要注意的是，大规格文件需要分片读取文件，将读取的文件内容添加到`spark-md5`的`hash`计算中，直到文件读取完毕，最后返回最终的`hash`码到`callback`回调函数里面。这里可以根据需要添加文件读取的进度条。
可以参考下面的代码:
